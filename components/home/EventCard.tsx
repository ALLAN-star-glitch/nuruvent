...
/**
 * Pick a lucide icon for an event type based on its slug.
 * Matches on substrings so new type variants fall through gracefully.
 */
...
export function EventCard({ event, onClick, featured = false }: EventCardProps) {
  ...
  // ---- Derived display values (from eventDisplay helpers) ----
  ...
  // ---- Visual glow (unchanged logic, now keyed on the new fields) ----
  const getGlowColor = () => {
