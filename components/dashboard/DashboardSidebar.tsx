...
export function DashboardSidebar({ role = 'host' }: DashboardSidebarProps) {
  ...
  return (
    ...
    >
      {/* Toggle Button Container - Whole bar toggles sidebar */}
      <div 
        onClick={(e) => {
          e.preventDefault();
          e.stopPropagation();
      ...
      >
        ...
        >
          {collapsed ? (
            <ChevronRight className="h-4 w-4 pointer-events-none text-gray-600" />
          ) : (
            <ChevronLeft className="h-4 w-4 pointer-events-none text-gray-600" />
          )}
        </Button>
      </div>
      ...
      <div className="p-3 border-t border-gray-100 bg-white">
        <button
          ...
          className={cn(
            'flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 w-full text-red-500 hover:bg-red-50 cursor-pointer'
          )}
        >
